...
func getPartOne(input []int, sum int) ([]int, int, error) {
	...
	if len(values) == 0 {
		return nil, 0, fmt.Errorf("failed to identify records matching required total")
	}

	return values, values[0]*values[1], nil
}

func getPartTwo(input []int, sum int) ([]int, int, error) {
	values := make([]int, 3)
	...
	return values, values[0]*values[1]*values[2], nil
}